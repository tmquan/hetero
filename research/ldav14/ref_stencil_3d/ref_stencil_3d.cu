...
int main(int argc, char **argv) {
  ...
  cudaMalloc((void **)&u1_d, (prod+1)*sizeof(float));
  cudaMalloc((void **)&U1_d, (prod+1)*sizeof(double));
  cudaMalloc((void **)&u2_d, (prod+1)*sizeof(float));
  cudaMalloc((void **)&U2_d, (prod+1)*sizeof(double));
  ...
  for (int prec=0; prec<2; prec++) {
    ...
    for (int pass=0; pass<4; pass++) {
      ...
      if (pass<3) {
        ...
        cudaFuncSetCacheConfig(BS_implicit2_x<32,32,double>, cudaFuncCachePreferShared);
        cudaFuncSetCacheConfig(BS_implicit2_y<32,32,double>, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(BS_implicit2_z<32,32,double>, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(BS_implicit2_rhs<32,32,double>, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(BS_implicit2_x<32,32,float>, cudaFuncCachePreferShared);
        cudaFuncSetCacheConfig(BS_implicit2_y<32,32,float>, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(BS_implicit2_z<32,32,float>, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(BS_implicit2_rhs<32,32,float>, cudaFuncCachePreferL1);
      }
...
//             U_h[indg] = fmax(0.0, fmin(i*dS, fmin(j*dS,k*dS)) - K);
      ...
      if (prec==0) {
       cudaMemcpy(u1_d,u_h, prod*sizeof(float) ,cudaMemcpyHostToDevice);
       cudaMemcpy(u2_d,u_h, prod*sizeof(float) ,cudaMemcpyHostToDevice);
      }
      else {
        cudaMemcpy(U1_d,U_h, prod*sizeof(double),cudaMemcpyHostToDevice);
...
//      imid = (NX/2+1) + (NY/2+1)*(NX+2) + (NZ/2+1)*(NX+2)*(NY+2);
      ...
      if (prec==0) {
        cudaMemcpy(u_h,u1_d,prod*sizeof(float), cudaMemcpyDeviceToHost);
...
//              if (i==NX/2 && k==NX/2) printf(" %d  %f \n",j,u_h[ind]-u_h[imid]);
        ...
        cudaMemcpy(U_h,U1_d,prod*sizeof(double), cudaMemcpyDeviceToHost);
