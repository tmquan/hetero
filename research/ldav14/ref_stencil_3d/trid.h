...
// an alternative version which swaps 1 reciprocal for 2 FMAs
// and 2 shuffles -- probably not a good swap based on 
// instruction throughputs in CUDA C Programming Guide

template <typename REAL>
__forceinline__ __device__ 
REAL trid1_warp_new(REAL a, REAL b, REAL c, REAL d){
  REAL bm, bp, ba, bc, bb;
  uint s=1;
...
#pragma unroll
  for (int n=0; n<5; n++) {
    ...
    bc = bm*c;
    bb = bm*bp;
    b  = bb*b  - ba*__shfl_up(c,s)
               - bc*__shfl_down(a,s);
    d  = bb*d  - ba*__shfl_up(d,s)
               - bc*__shfl_down(d,s); 
    a  =       - ba*__shfl_up(a,s);
    c  =       - bc*__shfl_down(c,s); 
...
//
// here we have the first part of the above routine
// which is needed for new work in 3D
//

template <int SIZE, typename REAL>
__forceinline__ __device__ 
void trid_warp_part1(REAL *a, REAL *b, REAL *c, REAL *d){
