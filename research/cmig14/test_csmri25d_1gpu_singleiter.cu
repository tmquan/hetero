...
int main(int argc, const char* argv[])
{
	...
	/// <summary>	Reserve Memory for the axillary variables. </summary>
	...
	// scale(d_cu, d_murf, dimx, dimy, dimz, Scale); 
	
	
	/// Prepare the kernel for convolution
	float2 *h_uker;
	h_uker = new float2[nTotal];
	memset(h_uker, 0, nTotal*sizeof(float2));
	for(int z=0; z<dimz; z++)
	{
		for(int y=0; y<dimy; y++)
		{
			...
			{
				if((x==0) && (y==0))		h_uker[at(x, y, z, dimx, dimy, dimz)] = make_float2(-4.0f, +0.0f);
				if((x==0) && (y==1))		h_uker[at(x, y, z, dimx, dimy, dimz)] = make_float2(+1.0f, +0.0f);
				if((x==1) && (y==0))		h_uker[at(x, y, z, dimx, dimy, dimz)] = make_float2(+1.0f, +0.0f);
				if((x==0) && (y==(dimy-1)))	h_uker[at(x, y, z, dimx, dimy, dimz)] = make_float2(+1.0f, +0.0f);
				if((x==(dimx-1)) && (y==0))	h_uker[at(x, y, z, dimx, dimy, dimz)] = make_float2(+1.0f, +0.0f);
			}
		}
	}
	float2* d_uker;
	cudaMalloc((void**)&d_uker, nTotal*sizeof(float2));							checkLastError();
	cudaMemset(d_uker, 0, nTotal*sizeof(float2));								checkLastError();
	cudaMemcpy(d_uker, h_uker, nTotal*sizeof(float2), cudaMemcpyHostToDevice);	checkLastError();
	dft(d_uker, d_uker, dimx, dimy, dimz, DFT_FORWARD, plan);					checkLastError();
	scale(d_mask, d_mask, dimx, dimy, dimz, Mu);								checkLastError();
	scale(d_uker, d_uker, dimx, dimy, dimz, Lambda_w);                          checkLastError();
	sub(d_mask, d_uker, d_uker, dimx, dimy, dimz);                              checkLastError();
	// cudaMemcpy(h_uker, d_uker, nTotal*sizeof(float2), cudaMemcpyDeviceToHost);	checkLastError(); //Debug
	// checkWriteFile(zero, h_uker, nTotal*sizeof(float2));
	...
	for(iOuter=0; iOuter<nOuter && isContinue ; iOuter++)
	{
		for(iInner=0; iInner<nInner && isContinue; iInner++)
		{
			...
			add(d_tx, d_ty, d_tz, d_tw, d_murf, d_rhs, dimx, dimy, dimz);//goto debug;
			
			///	Update u term using single iteration.
			dzt(d_cu, d_lz, dimx, dimy, dimz, DDT_LAPLACIAN);
			scale(d_lz, d_lz , dimx, dimy, dimz, Gamma_w);
			add(d_rhs, d_lz  , d_rhs, dimx, dimy, dimz); //goto debug;  //!!! Update right hand side
			
			dft(d_rhs, d_cu  , dimx, dimy, dimz, DFT_FORWARD, plan);//goto debug;
			div(d_cu , d_uker, d_cu, dimx, dimy, dimz); //goto debug;
			dft(d_cu , d_cu  , dimx, dimy, dimz, DFT_INVERSE, plan);//goto debug;
			scale(d_cu, d_cu , dimx, dimy, dimz, 1.0f/(dimx*dimy));//goto debug;
			// if(iInner==7) goto debug;
			// ///	Update u term using Richardson.
		...
		// scale(d_ft, d_ft, dimx, dimy, dimz, 1.0f/Scale);
		...
		// scale(d_murf, d_murf, dimx, dimy, dimz, Scale/(dimx*dimy)*Mu);
...
debug:
	...
	// cudaMemcpy(h_dest, d_uker, nTotal*sizeof(float2), cudaMemcpyDeviceToHost);
	// cudaMemcpy(h_dest, d_rhs, nTotal*sizeof(float2), cudaMemcpyDeviceToHost);
	checkLastError();
	/// Write the full reconstruction to binary file
