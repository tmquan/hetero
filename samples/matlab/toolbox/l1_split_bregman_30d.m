function u = l1_split_bregman_30d(R, f, params)
	%%  R       sampling mask
	...
	%% Initial u
	u = ifftn(f0);
    imagesc(abs(u(:,:,end/2))); colormap gray; axis square off; drawnow;
    ...
    for outer = 1:nOuter
		for inner = 1:nInner;
			...
			%% !!!
			rhs = murf - Lambda*tx - Lambda*ty - Gamma*(tz);
			% rhs = murf - lambda_w*(tx+ty) - gamma_w*(tz);
			
			%% !!!  Modified (Damped) Richardson iteration for solving linear system
			for loop = 1:nLoops;  
				Ax = Mu*ifftn(R.*fft2(u))       ...
                   + Lambda*Lx(u)               ...
                   + Lambda*Ly(u)               ...
                   + Gamma*Lz(u);
				u = u + Ep*(rhs - Ax);        
