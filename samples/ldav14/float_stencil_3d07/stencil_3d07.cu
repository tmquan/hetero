...
void stencil_3d07(float* deviceSrc, float* deviceDst, int dimx, int dimy, int dimz,  int blockx, int blocky, int blockz, int halo, cudaStream_t stream);

__global__ 
void __stencil_3d07(float* deviceSrc, float* deviceDst, int dimx, int dimy, int dimz, int halo);
...
void __stencil_3d07(float* deviceSrc, float* deviceDst, int dimx, int dimy, int dimz, int halo)
{
	...
	float tmp    =  beta*(sharedMemSrc[at(threadIdx.x + halo +1, threadIdx.y + halo+0, threadIdx.z + halo+0, sharedMemDim.x, sharedMemDim.y, sharedMemDim.z)] +
						  ...
						  sharedMemSrc[at(threadIdx.x + halo +0, threadIdx.y + halo+0, threadIdx.z + halo-1, sharedMemDim.x, sharedMemDim.y, sharedMemDim.z)]);
	float result = tmp+alpha*sharedMemSrc[at(threadIdx.x + halo, threadIdx.y + halo, threadIdx.z + halo, sharedMemDim.x, sharedMemDim.y, sharedMemDim.z)];                                                                                    
