...
void __stencil_3d(float* deviceSrc, float* deviceDst, int dimx, int dimy, int dimz, int halo)
{
	...
	//First pass will load entire 3 planes, process and write
	{
		...
		{                                                                                     										
			...
			shared_index_3d  =  make_int3((shared_index_1d % ((blockDim.y+2*halo)*(2*blockDim.x+2*halo))) % (2*blockDim.x+2*halo),		
										  (shared_index_1d % ((blockDim.y+2*halo)*(2*blockDim.x+2*halo))) / (2*blockDim.x+2*halo),		
										  (shared_index_1d / ((blockDim.y+2*halo)*(2*blockDim.x+2*halo))) );      					
			global_index_3d  =  make_int3(blockIdx.x *2*blockDim.x+shared_index_3d.x-halo, 										
		...
		for(batch=0; batch<numWorking; batch++)
		{
			...
			// global_index_1d  =  global_index_3d.z * dimy * dimx+                                   								
								// global_index_3d.y * dimx+                                   										
								// global_index_3d.x;        
			// result  = sharedMemSrc[at(shared_index_3d.x+halo+0, shared_index_3d.y+halo+0, shared_index_3d.z+halo+0, 
									  // sharedMemDim.x, sharedMemDim.y, sharedMemDim.z)];	
	...
	// index_3d       =  make_int3(blockIdx.x * blockDim.x*2+threadIdx.x+blockDim.x,                    
								// blockIdx.y * blockDim.y+threadIdx.y,                    
								// blockIdx.z * blockDim.z+threadIdx.z);                   
	// index_1d       =  index_3d.z * dimy * dimx+                                         
					  // index_3d.y * dimx+                                                
	...
	// if((index_3d.x >= dimx)||(index_3d.y>=dimy)||(index_3d.z>dimz))
	 ...
	 // // Multi batch reading here                                                           										
    // sharedMemDim    = make_int3(blockDim.x*2+2*halo+blockDim.x,                                   										
                                // blockDim.y+2*halo,                                  										
                                // blockDim.z+2*halo);                                  										
    // sharedMemSize   = sharedMemDim.x*sharedMemDim.y*sharedMemDim.z;                                    										
    // blockSize        = blockDim.x*blockDim.y*blockDim.z*2;                  									
    // numBatches       = sharedMemSize/blockSize+((sharedMemSize%blockSize)?1:0);    
	...
	// {
		...
		// {                                                                                     										
			// shared_index_1d  =  threadIdx.z * blockDim.y * blockDim.x*2+                      										
								// threadIdx.y * blockDim.x*2+                                   										
								// threadIdx.x + blockDim.x +                                              										
								// blockSize*batch; //Magic is here quantm@unist.ac.kr           										
			// shared_index_3d  =  make_int3((shared_index_1d % ((blockDim.y+2*halo)*(blockDim.x*2+2*halo))) % (blockDim.x*2+2*halo),		
										  // (shared_index_1d % ((blockDim.y+2*halo)*(blockDim.x*2+2*halo))) / (blockDim.x*2+2*halo),		
										  // (shared_index_1d / ((blockDim.y+2*halo)*(blockDim.x*2+2*halo))) );      					
			// global_index_3d  =  make_int3(blockIdx.x * blockDim.x*2+shared_index_3d.x-halo, 										
										  // blockIdx.y * blockDim.y+shared_index_3d.y-halo, 										
										  // blockIdx.z * blockDim.z+shared_index_3d.z-halo);										
			// global_index_1d  =  global_index_3d.z * dimy * dimx+                                   								
								// global_index_3d.y * dimx+                                   										
		...
		// // Single pass writing here                                                           
		// index_3d       =  make_int3(blockIdx.x * blockDim.x*2+threadIdx.x,                    
									// blockIdx.y * blockDim.y+threadIdx.y,                    
									// blockIdx.z * blockDim.z+threadIdx.z);                   
		// index_1d       =  index_3d.z * dimy * dimx+                                         
						  // index_3d.y * dimx+                                                
