...
int main(int argc, char *argv[])
{
	...
	// // Reallocate to 2d
	// float **t_src;
	// t_src = (float**)malloc(subDataDim.y*sizeof(float*));
	// for (int y = 0; y < subDataDim.y; ++y) 
		// t_src[y] = (float *)malloc(subDataDim.x * sizeof(float));
	
	// // Casting the pointer
	// for (int y = 0; y < subDataDim.y; ++y) 
		// // t_src[y] = &p_src[y*subDataDim.x];
		// memcpy(t_src[y], &p_src[y*subDataDim.x], subDataDim.x * sizeof(float));
	// // memcpy(t_src, p_src, subDataDim.x*subDataDim.y * sizeof(float));
	//================================================================================
	///!!! Extend the memory to form another buffer
	int3 extDataDim		{0, 0, 0};
	int halo = 5;
	extDataDim = make_int3(subDataDim.x + 2*halo,
						   subDataDim.y + 2*halo,
						   1);
	// Form another buffer
	float *p_ext;
	...
	// float **t_ext;
	// t_ext = (float**)malloc(extDataDim.y*sizeof(float*));
	// for (int y = 0; y < extDataDim.y; ++y) 
		// t_ext[y] = (float *)malloc(extDataDim.x * sizeof(float));
	...
	MPI_Type_commit(&subarray);
	// Self copy
	// MPI_Isend(p_src, 1, subarray, rank, MPI_ANY_TAG, MPI_COMM_WORLD, &request);	
	MPI_Isend(p_src, subDataDim.x*subDataDim.y, MPI_FLOAT, rank, 0, MPI_COMM_WORLD, &request);	
	// MPI_Send(t_src, 1, subarray, rank, 99, MPI_COMM_WORLD);	
	// MPI_Isend(t_src, 1, subarray, rank, 99, MPI_COMM_WORLD, &request);	
	// MPI_Isend(p_src, subDataDim.x*subDataDim.y, MPI_FLOAT, rank, 1, MPI_COMM_WORLD, &request);	
	cout << "Debug at " << __FILE__ << " " << __LINE__ << endl;
	// MPI_Recv(t_ext, 1, subarray, rank, 99, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	// MPI_Recv(p_ext, subDataDim.x*subDataDim.y, MPI_FLOAT, rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	MPI_Recv(p_ext, 1, subarray, rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	///!!! MPI SendRecv for self communicator doesnot work, check back later
	///!!! Mannual copy
	// for (int y = 0; y < subDataDim.y; ++y) 
		// memcpy(&t_ext[halo][y+halo], t_src[y], subDataDim.x*sizeof(float));
	
	// for (int y = 0; y < extDataDim.y; ++y) 
		// &p_ext[(y)*extDataDim.x] = t_ext[y];
		// memcpy(&p_ext[(y)*extDataDim.x], t_ext[y], extDataDim.x*sizeof(float));
	// MPI_Irecv(t_ext, 1, subarray, rank, 99, MPI_COMM_WORLD, &request);
	// MPI_Recv(&p_ext[0][0], 1, subarray, rank, 99, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	// MPI_Recv(&p_ext[at(halo, halo, extDataDim.x, extDataDim.y)], 1, subarray, rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	// MPI_Recv(&p_ext(halo*extDataDim.x + halo), 1, subarray, rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	// MPI_Recv(p_ext+(halo*extDataDim.x + halo), subDataDim.x*subDataDim.y, MPI_FLOAT, rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	cout << "Debug at " << __FILE__ << " " << __LINE__ << endl;	
	MPI_Barrier(MPI_COMM_WORLD);
